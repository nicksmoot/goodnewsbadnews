...
import { CAT, QueueItem, WorkflowState } from "@/lib/data";
import { cityPosts, cityQueue } from "@/lib/selectors";
...
function Badge({ item }: { item: QueueItem }) {
  const c = CAT[item.cat] || CAT.signal;
  return (
...
export default function AdminPage() {
  const { posts, queue, city, setWf, publishItem, rejectItem, resetDemo } = useStore();

  if (!config.showAdmin) {
    return (
      <section style={{ maxWidth: 760, margin: "0 auto", padding: "60px 24px 80px" }}>
        <h1 style={{ fontFamily: "'Spectral',serif", fontWeight: 800, fontSize: 32 }}>Moderation is disabled</h1>
        <p style={{ fontSize: 16, color: "#5a564d" }}>The moderation board is turned off in this build. In production, admin access is enforced server-side.</p>
      </section>
    );
  }

  const q = cityQueue(queue, city);
  ...
  const statPublished = cityPosts(posts, city).length;
  const statFlagged = q.filter((x) => x.scan && x.scan.level === "flag" && x.wf !== "Published").length;
    ...
    <div>
      <section style={{ maxWidth: 1340, margin: "0 auto", padding: "44px 24px 70px" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", flexWrap: "wrap", gap: 16, borderBottom: "1px solid #d8cab2", paddingBottom: 18, marginBottom: 24 }}>
          <div>
            <div style={{ fontFamily: "'IBM Plex Mono',monospace", textTransform: "uppercase", letterSpacing: "1.6px", fontSize: 12, color: "#6b675e", marginBottom: 10 }}>Moderation · Internal</div>
          ...
          <div style={{ display: "flex", gap: 18, fontFamily: "'IBM Plex Mono',monospace", fontSize: 12, color: "#6b675e" }}>
            ...
            <span>Published <strong style={{ color: "#161616", fontSize: 16 }}>{statPublished}</strong></span>
            <span>Flagged <strong style={{ color: "#a33429", fontSize: 16 }}>{statFlagged}</strong></span>
          </div>
        </div>
        <p style={{ fontSize: 15, lineHeight: 1.5, color: "#5a564d", maxWidth: 760, margin: "0 0 26px" }}>Every submission moves through automated risk screening and human review before publishing. Move a card right to advance it; publish to push it live to the public feed.</p>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 14, alignItems: "start" }}>
          ...
          <div style={colBox}>
            ...
            {colNew.map((item) => {
              const flag = item.scan && item.scan.level === "flag";
              return (
                <div key={item.id} style={{ background: "#fffaf1", border: "1px solid #d8cab2", borderRadius: 12, padding: 13, marginBottom: 10 }}>
                  ...
                  <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                    <button onClick={() => setWf(item.id, "Review" as WorkflowState)} style={{ ...smallBtn, border: "1px solid #d8cab2", background: "#fffdf8", color: "#161616" }}>Send to review &rarr;</button>
                    <button onClick={() => rejectItem(item.id)} style={{ ...smallBtn, border: "1px solid #a3342959", background: "#a3342914", color: "#a33429" }}>Reject</button>
                  </div>
                </div>
              );
            })}
          ...
          <div style={colBox}>
            ...
            {colReview.map((item) => (
              <div key={item.id} style={{ background: "#fffaf1", border: "1px solid #d8cab2", borderRadius: 12, padding: 13, marginBottom: 10 }}>
                ...
                <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                  <button onClick={() => setWf(item.id, "Verified" as WorkflowState)} style={{ ...smallBtn, border: "1px solid #19734a59", background: "#19734a14", color: "#19734a" }}>Mark verified &rarr;</button>
                  <button onClick={() => setWf(item.id, "New" as WorkflowState)} style={{ ...smallBtn, border: "1px solid #d8cab2", background: "#fffdf8", color: "#6b675e" }}>&larr; Back</button>
                  <button onClick={() => rejectItem(item.id)} style={{ ...smallBtn, border: "1px solid #a3342959", background: "#a3342914", color: "#a33429" }}>Reject</button>
                </div>
              </div>
            ))}
          ...
          <div style={colBox}>
            ...
            {colVerified.map((item) => (
              <div key={item.id} style={{ background: "#fffaf1", border: "1px solid #d8cab2", borderRadius: 12, padding: 13, marginBottom: 10 }}>
                ...
                <div style={{ ...metaLineStyle, marginBottom: 10 }}>{meta(item)}</div>
                <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                  <button onClick={() => publishItem(item.id)} style={{ ...smallBtn, border: "none", background: "#19734a", color: "#fff", padding: "6px 10px" }}>Publish to feed</button>
                  <button onClick={() => setWf(item.id, "Review" as WorkflowState)} style={{ ...smallBtn, border: "1px solid #d8cab2", background: "#fffdf8", color: "#6b675e" }}>&larr; Back</button>
                </div>
              </div>
            ))}
          </div>
        ...
        <div style={{ marginTop: 22 }}>
          <button onClick={resetDemo} style={{ border: "1px solid #d8cab2", background: "#fffdf8", color: "#6b675e", borderRadius: 999, padding: "9px 16px", fontSize: 12.5, fontWeight: 700, cursor: "pointer" }}>Reset demo data</button>
        </div>
      </section>
    </div>
  );
